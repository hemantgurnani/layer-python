...
LAYER_URI_WEBHOOK_ACTIVATE = 'activate'
LAYER_URI_WEBHOOK_DEACTIVATE = 'deactivate'
...
class WebhookClient(object):
    """
    Client to the Webhook REST API
    """

    def __init__(self, app_uuid, bearer_token):
        """
        Create a new Layer Webhook client.
    ...
    def get_webhook_details(self, webhook_uuid):
        """
    ...
    def register_webhook(self, json_data):
        """
        {
            "version": "1.0",
        ...
        return Webhook.from_dict(self._raw_request(
            METHOD_POST,
            self._get_layer_uri(
                LAYER_URI_WEBHOOKS,
                ),
        ...
        return Webhook.from_dict(
            self._raw_request(
                ...
                self._get_layer_uri(
                    LAYER_URI_WEBHOOKS,
                    webhook_uuid,
                    LAYER_URI_WEBHOOK_ACTIVATE
                    ),
                ),
        )


    def deactivate_webhook(self, webhook_uuid):
        """
        Fetch an existing webhooks by UUID
    ...
    def validate_webhook(self, endpoint_url, challenge):
        """
        """
        result = requests.request(
            METHOD_GET,
...
class Webhook(BaseLayerResponse):
    """
    Represents a Webhook conversation.
    """

    def __init__(self, id, url, version, target_url, events,
